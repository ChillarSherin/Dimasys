package com.chillarcards.dimasys.ui.others.total_billings

import android.util.Log
import androidx.lifecycle.LiveData
import androidx.lifecycle.MutableLiveData
import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import com.chillarcards.dimasys.data.model.total_billing.res.BillingsResModel
import com.chillarcards.dimasys.data.model.transaction.res.RecentTransactionsResModel
import com.chillarcards.dimasys.data.repository.AuthRepository
import com.chillarcards.dimasys.utills.NetworkHelper
import com.chillarcards.dimasys.utills.Resource
import kotlinx.coroutines.NonCancellable
import kotlinx.coroutines.launch

/**
 * Created by Deepak Dileepan on 12-07-2024.
 */
class TotalBillingViewModel(
    private val authRepository: AuthRepository,
    private val networkHelper: NetworkHelper
) : ViewModel() {

    private val _billings = MutableLiveData<Resource<BillingsResModel>?>()
    val billings: LiveData<Resource<BillingsResModel>?> get() = _billings

    var userId = MutableLiveData<String>()

    fun getPartnerRecentTransactions() {
        viewModelScope.launch(NonCancellable) {
            try {
                _billings.postValue(Resource.loading(null))
                if (networkHelper.isNetworkConnected()) {
                    authRepository.getBillings(
                        userId.value.toString(),
                    ).let {
                        if (it.isSuccessful) {
                            _billings.postValue(Resource.success(it.body()))
                        } else {
                            _billings.postValue(Resource.error(it.errorBody().toString(), null))
                        }
                    }
                } else {
                    _billings.postValue(Resource.error("No Internet Connection", null))
                }
            } catch (e: Exception) {
                Log.e("abc_otp", "verifyOTP: ", e)
            }
        }
    }

}